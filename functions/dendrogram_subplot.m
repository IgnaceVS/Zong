function [hout,T,perm] = dendrogram_subplot(plot_handle, Z,varargin)
% Modified version of MATLAB "dendrogram".
%
...
if(~horz)
    for count = 1:(numLeaves-1)
        h(count) = line(plot_handle, A(:,count),B(:,count),'color',col(count,:));
    end
    lims = [0 numLeaves+1 max(0,ymin-margin) (ymax+margin)];
    xlim(plot_handle, [.5 ,(n +.5)]);
    xticks(plot_handle, 1:n);
    xticklabels(plot_handle, label);
    
    mask = logical([0 0 1 1]);
else
    for count = 1:(numLeaves-1)
        h(count) = line(plot_handle,B(:,count),A(:,count),'color',col(count,:));
    end
    lims = [max(0,ymin-margin) (ymax+margin) 0 numLeaves+1 ];
    ylim(plot_handle, [.5 ,(n +.5)]);
    yticks(plot_handle, 1:n);
    yticklabels(plot_handle, label);    
